...
export default function Settings() {
  ...
  return (
    <PageLayout variant="dashboard" title="Configurações" showBackButton headerActions={headerActions}>
        ...
        <form onSubmit={handleSubmit(handleSave)} className="space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  {qualityScore && usernameCheck.status === 'available' && usernameValue !== user?.username && (
                    <div className="flex items-center gap-2"><Badge variant="outline" style={{ borderColor: qualityScore.color, color: qualityScore.color }} className="text-xs">{qualityScore.badge === 'premium' && '⭐ Premium'}{qualityScore.badge === 'good' && '✓ Bom'}{qualityScore.badge === 'average' && 'Regular'}{qualityScore.badge === 'poor' && 'Fraco'}</Badge><span className="text-xs text-muted-foreground">Qualidade: {qualityScore.score}%</span></div>
                  )}
                  {usernameCheck.suggestions.length > 0 && usernameValue !== user?.username && (
                    <div className="space-y-2"><p className="text-xs text-muted-foreground flex items-center gap-1"><Lightbulb className="h-3 w-3" />Sugestões disponíveis:</p><div className="flex flex-wrap gap-2">{usernameCheck.suggestions.map((suggestion) => (<button key={suggestion} type="button" onClick={() => applySuggestion(suggestion)} className="px-2 py-1 text-xs rounded-md bg-accent/20 hover:bg-accent/30 text-foreground/80 hover:text-foreground transition-colors">@{suggestion}</button>))}</div></div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {(["light", "dark", "neon"] as const).map((themeOption) => (
                  <div key={themeOption} className={`p-4 rounded-lg border-2 cursor-pointer ${theme === themeOption ? "border-primary" : "border-border"}`} onClick={() => setTheme(themeOption)}>
                    <div className="flex flex-col items-center gap-3"><div className={`w-16 h-10 rounded-md ${themeOption === "light" ? "bg-white" : themeOption === "dark" ? "bg-gray-900" : "bg-gradient-neon"}`} /><p className="font-medium capitalize flex items-center gap-2">{themeOption} {theme === themeOption && <CheckCircle className="h-4 w-4 text-primary" />}</p></div>
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card className="glass-card border-accent/20">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-lg bg-card/30"><div><h4 className="font-medium">Alterar senha</h4><p className="text-sm text-muted-foreground">Recomendamos trocar sua senha regularmente</p></div><Button type="button" variant="outline" onClick={() => setPasswordModalOpen(true)}>Alterar</Button></div>
              <div className="flex items-center justify-between p-4 rounded-lg bg-card/30"><div><h4 className="font-medium">Sair da conta</h4><p className="text-sm text-muted-foreground">Desconectar sua sessão em todos os dispositivos</p></div><Button type="button" variant="outline" onClick={() => setLogoutModalOpen(true)}><LogOut className="h-4 w-4 mr-2" />Sair</Button></div>
            </CardContent>
          </Card>
      ...
      <ConfirmDeleteModal open={deleteModalOpen} onOpenChange={setDeleteModalOpen} onConfirm={handleDeleteAccount} itemName="sua conta" />
